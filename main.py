...
# ---------- НОВАЯ ФУНКЦИЯ: ПОИСК ПО ФИО (ссылки на гос. ресурсы + соцсети + Google Dorks) ----------
async def get_fio_info(fio: str):
    """Генерирует ссылки для поиска по ФИО на гос. ресурсах, в соцсетях и по Google Dorks."""
    if not fio or len(fio) < 3:
        return None, "❌ Введите более 3 символов для поиска."

    # Кодируем ФИО для вставки в URL
    encoded_fio = urllib.parse.quote(fio)
    # Для некоторых сайтов нужен другой формат (например, для Google Dorks)
    fio_parts = fio.split()
    last_name = fio_parts[0] if len(fio_parts) > 0 else ""
    first_name = fio_parts[1] if len(fio_parts) > 1 else ""
    middle_name = fio_parts[2] if len(fio_parts) > 2 else ""

    # Словарь с названиями сайтов и URL для поиска
    sites = {
        "🏛 Банкроты (Федресурс, РФ)": f"https://bankrot.fedresurs.ru/BankruptSearch.aspx?searchString={encoded_fio}&searchType=Name",
        "🏛 Должники (Беларусь)": f"https://bankrot.gov.by/Debtors/DebtorsList/?fullname={encoded_fio}",
        "⚖️ Судебные решения (OpenDataBot)": f"https://court.opendatabot.ua/#/search?query={encoded_fio}",
        "📋 ЕГР (Беларусь)": f"http://egr.gov.by/egrn/index.jsp?content=Find&lastname={last_name}&firstname={first_name}&secondname={middle_name}",
        "🇪🇺 База должников (ЕС)": "https://e-justice.europa.eu/content_presentation-246-ee.do?clang=en&idTaxonomy=246&m=1",
        "🇰🇿 Задолженности (Казахстан)": f"https://fa-fa.kz/ru/search?query={encoded_fio}",
        "⚖️ ФССП (РФ)": f"http://fssprus.ru/iss/ip?query={encoded_fio}",
        "⚖️ Решения судов (РФ)": f"https://www.gcourts.ru/search?q={encoded_fio}",
        "🏷 Реестр товаров (Беларусь)": f"https://gr.rcheph.by/ru/registry?query={encoded_fio}",
        "🏛 Ликвидация юрлиц (Беларусь)": f"https://justbel.info/Liquidation/FindMyRequest?query={encoded_fio}",
        "⚖️ Арбитраж (РФ)": f"https://kad.arbitr.ru/?query={encoded_fio}",
        "🇰🇿 Неблагонадежные налогопл. (Казахстан)": f"http://kgd.gov.kz/ru/services/taxpayer_search_unreliable?query={encoded_fio}",
        "🇺🇦 Реестр должников (Украина)": f"https://erb.minjust.gov.ua/#/search-debtors?fullName={encoded_fio}",
        "📄 Поиск в документах (MMNT)": f"https://www.mmnt.ru/int/search?query={encoded_fio}",
        "🚛 Транспортные лицензии (Беларусь)": f"http://mtkrbti.by/local/TL/Licence.nsf/WEBSearchView1?SearchView&Query=[FIO]%3D{encoded_fio}",
        "👮 Розыск (МВД Беларусь)": f"https://www.mvd.gov.by/ru/wanted?name={encoded_fio}",
        "📄 Поиск ИНН (ФНС РФ)": "https://service.nalog.ru/inn.do",
        "📜 Наследственные дела (РФ)": f"https://data.notariat.ru/directory/succession/search?last_name={last_name}&first_name={first_name}&middle_name={middle_name}#",
        "🇧🇾 Портал налоговой (Беларусь)": f"https://portal.nalog.gov.by/grp/?query={encoded_fio}",
        "📃 Реестр доверенностей (РФ)": f"https://reestr-dover.ru/search?q={encoded_fio}",
    }

    # Google Dorks для поиска по ФИО
    dorks = {
        "📧 Email (утечки)": f"https://www.google.com/search?q=\"{encoded_fio}\"+email+OR+%40+OR+mail",
        "📞 Телефон": f"https://www.google.com/search?q=\"{encoded_fio}\"+phone+OR+tel+OR+%2B7",
        "👥 Профили в соцсетях": f"https://www.google.com/search?q=\"{encoded_fio}\"+site%3Avk.com+OR+site%3Aok.ru+OR+site%3Afacebook.com+OR+site%3Ainstagram.com",
        "📄 Документы (PDF, DOC)": f"https://www.google.com/search?q=\"{encoded_fio}\"+filetype%3Apdf+OR+filetype%3Adoc+OR+filetype%3Adocx",
        "🔍 Упоминания в интернете": f"https://www.google.com/search?q=\"{encoded_fio}\""
    }

    # Социальные сети (страницы восстановления/поиска)
    socials = {
        "👤 ВКонтакте": f"https://vk.com/people?q={encoded_fio}",
        "📸 Instagram": "https://www.instagram.com/accounts/password/reset",
        "📘 Facebook": "https://facebook.com/login/identify/?ctx=recover&ars=royal_blue_bar",
        "🐦 Twitter": "https://twitter.com/account/begin_password_reset",
        "🔗 LinkedIn": "https://linkedin.com/checkpoint/rp/request-password-reset-submit",
        "📺 Одноклассники": "https://ok.ru/dk?st.cmd=anonymRecoveryStartPhoneLink",
        "🎵 TikTok": f"https://www.tiktok.com/search?q={encoded_fio}",
        "💬 Telegram": f"https://t.me/{encoded_fio}"
    }

    # Формируем итоговое сообщение
    lines = [f"🔎 **Поиск по ФИО: `{fio}`**\n"]
    lines.append("**🏛 Государственные ресурсы:**")
    for name, url in sites.items():
        lines.append(f"• [{name}]({url})")

    lines.append("\n**🔍 Google Dorks (утечки, документы, упоминания):**")
    ...
    lines.append("\n_💡 Ссылки ведут на страницы поиска. На некоторых сайтах потребуется ввести дополнительные данные (дату рождения, паспорт и т.п.). Если сайт не открывается, попробуйте позже или используйте VPN._")
    ...
    try:
        if action == "nick":
            ...
            if not text:
                ...
                return TYPING_FIO
            await update.message.reply_text(f"⏳ Формирую ссылки для поиска по ФИО '{text}'...")
            info, err = await get_fio_info(text)
            if err:
                await update.message.reply_text(err)
            else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Искать данные по ФИО (ссылки на гос. ресурсы, соцсети и Google Dorks)\n"
...
def main():
    ...
    print("Бот запущен и готов к работе (финальная версия с поиском по ФИО)")
