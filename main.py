...
(CHOOSING, TYPING_NICK, TYPING_TG_USERNAME, TYPING_IP,
 ...
 TYPING_FIO, TYPING_TELEGRAM_ID,
 TYPING_ADMIN_USER_ID, TYPING_ADMIN_AMOUNT) = range(15)
...
# ---------- Улучшенная функция поиска по номеру телефона (старая) ----------
...
# ---------- НОВЫЕ ФУНКЦИИ ИЗ КОНСОЛЬНОГО СКРИПТА ----------

# 1. Поиск по ФИО (ofdata.ru)
...
# 2. Поиск по номеру телефона через Veriphone
async def get_phone_info_veriphone(phone: str):
    if not VERIPHONE_API_KEY:
        return None, "❌ API-ключ Veriphone не настроен. Добавьте VERIPHONE_API_KEY в .env"
    # Очищаем номер от лишних символов, оставляем только цифры и плюс
    clean_phone = re.sub(r'[^\d+]', '', phone)
    if not clean_phone:
        return None, "❌ Некорректный номер"
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status != 200:
                    return None, f"❌ Ошибка Veriphone API: {resp.status}"
                data = await resp.json()
                if data.get('status') != 'success':
                    return None, "❌ Не удалось получить данные."
                items = {}
                if data.get('phone'):
                    items["Телефон"] = data['phone']
                if data.get('carrier'):
                    items["Оператор"] = data['carrier']
                if data.get('phone_valid'):
                    items["Валидный"] = "Да" if data['phone_valid'] else "Нет"
                if data.get('phone_type'):
                    items["Тип номера"] = data['phone_type']
                if data.get('phone_region'):
                    items["Регион"] = data['phone_region']
                if data.get('country'):
                    items["Город/страна"] = data['country']
                if data.get('country_code'):
                    items["Код страны"] = data['country_code']
                if data.get('country_prefix'):
                    items["Префикс"] = data['country_prefix']
                if data.get('international_number'):
                    items["Международный формат"] = data['international_number']
                if data.get('local_number'):
                    items["Локальный формат"] = data['local_number']
                return format_dict_as_damage(items, title=f"📞 Результаты Veriphone по номеру {phone}"), None
    except asyncio.TimeoutError:
        return None, "❌ Таймаут при запросе к Veriphone"
    except Exception as e:
        return None, f"❌ Ошибка: {e}"

# 3. Поиск по email через AbstractAPI (валидация + репутация)
async def get_email_info_abstract(email: str):
    if not EMAIL_VALIDATION_API_KEY or not EMAIL_REPUTATION_API_KEY:
        return None, "❌ Ключи AbstractAPI не настроены."
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(val_url, timeout=10) as val_resp, session.get(rep_url, timeout=10) as rep_resp:
        ...
        if not val_data:
            return None, "❌ Ошибка валидации email."
        items = {}
        ...
        return format_dict_as_damage(items, title=f"📧 Результаты для {email}"), None
    except asyncio.TimeoutError:
        return None, "❌ Таймаут при запросе к AbstractAPI"
    except Exception as e:
        return None, f"❌ Ошибка: {e}"

# 4. Поиск по IP через несколько источников (как в скрипте)
IP_APIS = [
    {"name": "ipinfo.io", "url": "https://ipinfo.io/{ip}/json", "method": "GET"},
    {"name": "ipwhois.io", "url": "https://ipwhois.app/json/{ip}", "method": "GET"},
    {"name": "freegeoip.app", "url": "https://freegeoip.app/json/{ip}", "method": "GET"},
]

async def get_ip_info_multiple(ip: str):
    if not is_ip(ip):
...
# 5. Поиск по Telegram ID в локальной папке base
async def search_by_telegram_id_file(tg_id: str):
    base_dir = "base"
    if not os.path.exists(base_dir):
        return None, "❌ Папка 'base' не найдена. Поместите CSV-файлы в папку base."
    found = False
    output_lines = [f"🔍 Результаты поиска по Telegram ID {tg_id}"]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🌐 Расширенный IP", callback_data="ip_multi")],
        [InlineKeyboardButton("🐙 GitHub по username", callback_data="github_user")],
        [InlineKeyboardButton("📧 Поиск по email", callback_data="email")],
        [InlineKeyboardButton("📧 Email (AbstractAPI)", callback_data="email_abstract")],
        ...
        [InlineKeyboardButton("📞 Veriphone", callback_data="phone_veri")],
        ...
        [InlineKeyboardButton("👤 Поиск по ФИО (ofdata)", callback_data="fio")],
        [InlineKeyboardButton("📁 Поиск по Telegram ID (локальный)", callback_data="telegram_id")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "nick":
        ...
        return TYPING_IP
    elif action == "ip_multi":
        await query.edit_message_text("Введите IP-адрес для расширенного поиска:")
        return TYPING_IP  # используем то же состояние, но действие отличим по user_state
        ...
        await query.edit_message_text("Введите email для проверки утечек (LeakCheck, Hudson и др.):")
        return TYPING_EMAIL
    elif action == "email_abstract":
        await query.edit_message_text("Введите email для проверки через AbstractAPI:")
        return TYPING_EMAIL
    elif action == "domain":
        await query.edit_message_text("Введите домен (например, example.com):")
        return TYPING_DOMAIN
    elif action == "phone":
        await query.edit_message_text("Введите номер телефона в международном формате (например, +79123456789):")
        return TYPING_PHONE
    elif action == "phone_veri":
        await query.edit_message_text("Введите номер телефона для поиска через Veriphone:")
        return TYPING_PHONE
    elif action == "mnp":
        ...
        return TYPING_FIO
    elif action == "telegram_id":
        await query.edit_message_text("Введите Telegram ID (число):")
        return TYPING_TELEGRAM_ID
    elif action == "profile":
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if action == "nick":
            ...
            # Стандартный IP через ip-api.com
            ...
            # Расширенный IP через несколько API
            await update.message.reply_text(f"⏳ Выполняю расширенный поиск по IP {text}...")
            info, err = await get_ip_info_multiple(text)
            if err:
            ...
            # Стандартный email через LeakCheck и др.
            if not is_email(text):
                await update.message.reply_text("❌ Некорректный email. Попробуйте снова.")
                return TYPING_EMAIL
            await update.message.reply_text(f"⏳ Проверяю email {text} через LeakCheck и другие источники...")
            async with aiohttp.ClientSession() as session:
                tasks = [
            ...
            # Новый email через AbstractAPI
            info, err = await get_email_info_abstract(text)
            if err:
                await update.message.reply_text(err)
            else:
                await update.message.reply_text(info)
            ...
            if not is_phone(text):
                ...
                return TYPING_PHONE
            await update.message.reply_text(f"⏳ Анализирую номер {text} через htmlweb...")
            ...
            # Новый поиск через Veriphone
            info, err = await get_phone_info_veriphone(text)
            if err:
                await update.message.reply_text(err)
            else:
                await update.message.reply_text(info)
            ...
            if not text.isdigit():
                ...
                return TYPING_TELEGRAM_ID
            await update.message.reply_text(f"⏳ Ищу данные по Telegram ID {text} в локальной папке...")
            info, err = await search_by_telegram_id_file(text)
            if err:
                await update.message.reply_text(err)
            else:
                if len(info) <= 4096:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• Показывать информацию по IP (стандартную и расширенную)\n"
        ...
        "• Анализировать номер телефона (htmlweb.ru и Veriphone)\n"
        ...
        "• Искать Telegram ID в локальной папке base\n"
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            TYPING_TELEGRAM_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_input)],
    ...
    print("Бот запущен и готов к работе (все API интегрированы, реферальная программа активна)")
