...
(CHOOSING, TYPING_NICK, TYPING_TG_USERNAME, TYPING_IP,
 ...
 TYPING_ADMIN_USER_ID, TYPING_ADMIN_AMOUNT) = range(12)
...
# ---------- УЛУЧШЕННАЯ ФУНКЦИЯ ПОИСКА ПО НОМЕРУ (ИЗ NOBLACK-MAIL) ----------
async def get_phone_info(phone: str):
    ...
    if htmlweb_data.get('limit') is not None:
        items["Остаток лимитов"] = htmlweb_data['limit']
        ...
        async with session.get(url, timeout=10) as resp:
            if resp.status == 200:
                ...
                if 'country' in data:
                    ...
                    result['currency_code'] = data['country'].get('iso', '')  # приблизительно
                # Данные номера (блок '0')
                if '0' in data:
                    result['operator'] = {
...
async def _phoneradar_rating(phone: str):
    """Получает оценку номера с сайта phoneradar.ru"""
...
# ---------- НОВАЯ ФУНКЦИЯ: ПОИСК MNP ----------
async def get_mnp_info(phone: str):
    """Поиск MNP (переносимость номера) через htmlweb.ru/json/mnp/phone/"""
    clean_phone = re.sub(r'[^\d+]', '', phone)
    if not clean_phone:
        return None, "❌ Некорректный номер"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if data.get('limit') == 0:
                        return None, "❌ Израсходованы все лимиты"
                    items = {}
                    if 'city' in data:
                    ...
                    if 'oper' in data:
                        ...
                        if oper.get('url'):
                            oper_str += f" - {oper['url']}"
                        items["Оператор"] = oper_str
                    items["Остаток лимитов"] = data.get('limit', 0)
...
def main():
    ...
    print("Бот запущен и готов к работе (улучшенный поиск по номеру, MNP, TikTok)")
